...
my_dir = "C:\\Users\\kayle\\Documents\\Morton-REU Simulations\\Attempt4_full_factorial_100reps\\lowMig_lowSamp"
setwd(my_dir)


#list of scenarios
#simulation sub-folder directories
scenarios = c("\\scen1_lowMig_lowSamp",
              "\\scen2_lowMig_lowSamp",
              "\\scen3_lowMig_lowSamp",
              "\\scen4_lowMig_lowSamp",
              "\\scen5_lowMig_lowSamp",
              "\\scen6_lowMig_lowSamp",
              "\\scen7_lowMig_lowSamp",
              "\\scen8_lowMig_lowSamp",
              "\\scen9_lowMig_lowSamp")
...
#total alleles
total_alleles_lowMig_lowSamp_equal = array(0, dim = c(9,100))
#expected heterozygosity
heterozygosity_lowMig_lowSamp_equal = array(0, dim = c(9,100))
...
for(i in 1:length(scenarios)) {
  ...
  for(j in 1:length(list_files)) {
    ...
    total_alleles_lowMig_lowSamp_equal[i,j] = total_alleles
    
    #heterozygosity
    sum_temp_genind = summary(temp_genind)
    heterozygosity_lowMig_lowSamp_equal = sum_temp_genind$Hexp
    
    #saving results
    results_lowMig_lowSamp_equal[i,j] = sample_n_alleles/total_alleles
  }
}

#look at results
round(results_lowMig_lowSamp_equal, 3)
#total alleles
total_alleles_lowMig_lowSamp_equal
mean(total_alleles_lowMig_lowSamp_equal)
#heterozygosity
heterozygosity_lowMig_lowSamp_equal
mean(heterozygosity_lowMig_lowSamp_equal)
...
#total alleles
total_alleles_lowMig_lowSamp_prop = array(0, dim = c(9,100))
#expected heterozygosity
heterozygosity_lowMig_lowSamp_prop = array(0, dim = c(9,100))
...
for(i in 1:length(scenarios)) {
  ...
  for(j in 1:length(list_files)) {
    ...
    total_alleles = ncol(temp_genind@tab)
    total_alleles_lowMig_lowSamp_prop[i,j] = total_alleles
    
    #heterozygosity
    sum_temp_genind = summary(temp_genind)
    heterozygosity_lowMig_lowSamp_prop = sum_temp_genind$Hexp
    
    #calculating proportion and saving the results
    results_lowMig_lowSamp_prop[i,j] = sample_n_alleles/total_alleles
  }
}

#look at results
round(results_lowMig_lowSamp_prop, 3)
#total alleles
total_alleles_lowMig_lowSamp_prop
mean(total_alleles_lowMig_lowSamp_prop)
#heterozygosity
heterozygosity_lowMig_lowSamp_prop
mean(heterozygosity_lowMig_lowSamp_prop)
#***********************************************************************************************************************************************************
#saving results
...
save(total_alleles_lowMig_lowSamp_equal, total_alleles_lowMig_lowSamp_prop, file="total_alleles_lowMig_lowSamp.Rdata")
save(heterozygosity_lowMig_lowSamp_equal, heterozygosity_lowMig_highSamp_prop, file="heterozygosity_lowMig_lowSamp.Rdata")
#use load() to load in data
#***********************************************************************************************************************************************************
#converting results arrays to matrices
#Equal results array conversion:
results_plot_equal = as.data.frame(results_lowMig_lowSamp_equal)
...
results_plot_prop = as.data.frame(results_lowMig_lowSamp_prop)
...
ggplot(combined_results, aes(x=factor(scenario), y=prop_all, fill=strategy, color=factor(scenario))) + 
  geom_boxplot() +
  ggtitle("Low migration/low sampling") +
  ylim(0.85,1) +
  scale_fill_brewer() +
  theme_bw()
...
#testing assumptions
#normality
aov_results_lowMig_lowSamp = aov(prop_all ~ scenario + strategy, data = combined_results)
aov_residuals_lowMig_lowSamp = residuals(object=aov_results_lowMig_lowSamp)
shapiro.test(x=aov_residuals_lowMig_lowSamp)#non-normal data
ggplot(combined_results, aes(x=prop_all)) + geom_histogram(bins=20)
#visualizing results with Q-Q plot
plot(aov_results_lowMig_lowSamp, 2)

#equal variances
leveneTest(prop_all ~ as.factor(strategy)*as.factor(strategy), data = combined_results)
#visualizing results with residuals vs. fits plot
plot(aov_results_lowMig_lowSamp, 1)
#***************************************************************************************************************************************************************
...
p_values_lowMig_lowSamp = c(rep(0,9))
for(i in 1:length(scenarios)){
  x_var = results_lowMig_lowSamp_prop[i,]
  y_var = results_lowMig_lowSamp_equal[i,]
  test_result = wilcox.test(x_var, y_var)
  p_values_lowMig_lowSamp[i] = test_result$p.value
}
#print p-values
round(p_values_lowMig_lowSamp, 8)
#****************************************************************************************************************************************************************
#saving results
setwd("C:\\Users\\kayle\\Documents\\Morton-REU\\Attempt4_full_factorial_100rep\\R scripts")
save(p_values_lowMig_lowSamp, file="p_values_lowMig_lowSamp.Rdata")
